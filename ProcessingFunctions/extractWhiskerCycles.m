function [pump_struct,allJobs] = extractWhiskerCycles(allJobs,cont_thresh_px)
% Given the trials in allJobs, generate pump-struct, a struct whose units
% are cycles with relevant data associated, and label them apropriately 
% Used when defining approach phase
visFlag = 0; % Visualization of results
min_frames_cont = 3;
% Define low-pass and high-pass filter parameters
lp_freq = 80;
hp_freq = 7;
hp_filt = struct();
lp_filt = struct();
...
min_trace_len = 0; % Exclude shorter traces (not used corrently)
max_frames_interp = Inf; % How much interpolation frames to allow (all ok currently)
max_interp_prct = Inf; % What percent interpolation is allowed (all ok currently)
max_cycle_dist = 10; % Two cycles are not consecutive if the end of one is more than this number of frames from the start of the other

for trial_num = 1:length(allJobs) % Iterate over trials
    for side_num = 1:2 % iterate over side
        allJobs{trial_num}.Tracks.cycles_processed(side_num) = 0;
        not_nan_frames = sum(~isnan(allJobs{trial_num}.Tracks.angles))>1;
        if (sum(not_nan_frames)./length(not_nan_frames))<0.1 % If most observations are NaN, we drop the trial
            disp(['Bad tracking, trial: ',num2str(trial_num)])
            continue
        end
        allJobs = getAnglePhase(allJobs,trial_num,side_num,hp_filt,lp_filt); % Assign phase signal to trial and side
        allJobs = getCycleFromPhase(allJobs,trial_num,side_num); % Assign cycle limits
        if visFlag % If desired, visualize cycle segmentation
            visualizeCycles(allJobs,trial_num,side_num); 
            pause
            clf
        end
        allJobs{trial_num}.Tracks.cycles_processed(side_num) = 1; % Flag that processing was ok
    end
end

pump_struct = generatePumpStruct(allJobs); % Make a struct where units are cycles
pump_struct = markCycles(pump_struct, allJobs,cont_thresh_px,min_frames_cont); % Assign cycles with desired labels
pump_struct = relateConsecutiveCycles(pump_struct,min_trace_len,max_frames_interp,max_interp_prct,max_cycle_dist) % Assign information about previous/following cycles
end

%% Functions called

function [allJobs] = getAnglePhase(allJobs,trial_num,side_num,hp_filt,lp_filt)
% Get the phase of a signal
side = mode(allJobs{trial_num}.Tracks.side,2) == side_num;
side = side./side;
ang = abs(allJobs{trial_num}.Tracks.angles.*side);

% Estimate the common angle. To reduce noise  we use median. Instead of
% using median on the signals, we use them on their derivatives, to
% emphasize commonalities, and reduce the  effect of whiskers coming
% in and out of tracing.
...
med_ang = med_ang+nanmean(abs(ang(:))); % Bring back to the 'scale' of original whiskers

lp_med_ang = filtWithNan(med_ang,lp_filt); % Lowpass signal
bp_med_ang = filtWithNan(lp_med_ang,hp_filt); % Highpass to get 'bandpass-like'
nan_vec = sum(~isnan(ang),1)>1;
nan_vec = nan_vec./nan_vec;

% Get phase signal from Hilbert Transform
...
% Segement cycles from phase
phase_sig = allJobs{trial_num}.Tracks.cycles{side_num}.phase_sig; % Phase
lp_med_ang = allJobs{trial_num}.Tracks.cycles{side_num}.lp_med_ang; % lowpass angles
[peak_vals,peak_idx] = findpeaks(phase_sig); % Find peaks in phase
peak_idx(abs(peak_vals-pi)>(pi/8)) = []; % only use those close to pi
[trough_vals,trough_idx] = findpeaks(-1*phase_sig); % Find troughs
trough_idx(abs(trough_vals-pi)>(pi/8)) = [];
peak_idx = [peak_idx;ones(size(peak_idx))];
trough_idx = [trough_idx;-1*ones(size(trough_idx))];
peaks_and_troughs = [peak_idx, trough_idx]; % All peaks and troughs
[~,sort_idx] = sort(peaks_and_troughs(1,:)); % Sort pairs by peaks
peaks_and_troughs = peaks_and_troughs(:,sort_idx); 
extremum_class = [peaks_and_troughs(2,1:end-1);peaks_and_troughs(2,2:end)]'; % This and next line removes sequential extremums that are of the same type (e.g. local minima followed by global minima)
extremum_class = 1+find(all(extremum_class == [1,-1],2));
cycle_starts = peaks_and_troughs(1,extremum_class); % Define cycle starts
first_cycle_start = peaks_and_troughs(1,find(peaks_and_troughs(2,:) == -1,1));
if first_cycle_start ~= cycle_starts(1) % Correct errors
    cycle_starts = [first_cycle_start, cycle_starts];
end

% Correct cycle starts to peak location on lowpass signals
peak_idx = -1+find((multiStepDiff(sign(multiStepDiff(lp_med_ang,1)),1))>0);
[~,idx] = min(abs(peak_idx'-cycle_starts));
if length(unique(idx))<length(cycle_starts) % Remove duplicates caused by this process
...
% Also define edges as cycles, but note them as incomplete cycles
...
% Get sign of protraction retraction from the lowpass signal
sign_vec = [];
for kk = [-1,1]
...
function [] = visualizeCycles...
    (allJobs,trial_num,side_num)
% Visualize to examine fidelity; angles + median signals + phase + cycle
% parsing
...
for panel_num = 1:3
    ...
    hold on
    plot(trim_idx.*[1,1],cumsum(y_lim),'--','Color',[0.1 0.5 0.1],'LineWidth',2)    
end
...
% Generate a struct whose units are cycles, to facilitate processing
counter = 1;
for trial_num = 1:length(allJobs) % Iterate over trials
    for side_num = 1:2 % Iterate over sides
        if allJobs{trial_num}.Tracks.cycles_processed(side_num) % Only use successfully processed data
            for cycle_num = 1:size(allJobs{trial_num}.Tracks.cycles{side_num}.cycle_start_end,1) % Iterate over cycles
                pump_struct(counter).trial_num = trial_num;
                pump_struct(counter).side_num = side_num;
                
                % Assign all desired attributes of the cycle to a single
                % entry
                cycles = allJobs{trial_num}.Tracks.cycles{side_num};
...
function [pump_struct] = markCycles(pump_struct,allJobs,cont_thresh_px,min_frames_cont)
...
for pump_num = 1:length(pump_struct)
    ...
    % Is it during approach
    pump_struct(pump_num).during_approch = ...
        pump_struct(pump_num).start_end(1) < allJobs{trial_num}.Tracks.trim_idx;
    ...
    cont = wh_obj_dist<cont_thresh_px;
    cont = cont./cont;
    pump_cont = cont.*t;
